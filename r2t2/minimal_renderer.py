...
class MinimalRenderer:
    ...
    def collide(
        ...
        while True:
            ...
            y += dt * dy
            z += dt * dz
            if x <= 0.0 or x >= self.w_map or y <= 0.0 or y >= self.h_map or z >= self.max_value:
                result = 1.0
                break
    ...
    @ti.kernel
    def render(self, output_field: ti.types.ndarray(), azimuth: float, altitude: float):
        dz = ti.sin(altitude)
        dr = ti.cos(altitude)
        dx = ti.cos(azimuth) * dr
        dy = ti.sin(azimuth) * dr
        for i, j in output_field:
            x = i + ti.random(float)
            y = j + ti.random(float)
            output_field[i, j] = self.collide(x, y, dx, dy, dz)
